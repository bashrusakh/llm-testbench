...
- **Two silent `except Exception` blocks narrowed** — the result-record loader and the export-record loader now catch `(OSError, ValueError)` only. The remaining broad `except Exception` blocks are outer-coroutine boundaries (job runner, request handlers, model discovery) where the catch-all is intentional.
